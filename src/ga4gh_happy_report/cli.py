...
#     lib/d3 asset is no longer required.
...
#     *.jinja2.html templates) ship as package data under
#     ga4gh_happy_report/assets/html/ and are resolved via importlib.resources.
...
# Path setup
# ---------------------------------------------------------------------------
TEMPLATEDIR = str(importlib.resources.files(__package__) / "assets" / "html")
...
class CdnFallbackLoader(jinja2.BaseLoader):
    ...
    roc.js, report.css) ship as package data, while the heavy third-party libs
    are pulled from CDN without needing to be vendored.
    """
